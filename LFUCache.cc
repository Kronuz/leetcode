/*
c++ -std=c++17 -pedantic -Wall -Wextra -g -fno-omit-frame-pointer -fsanitize=address -fsanitize=undefined -fno-sanitize=vptr,function -fno-sanitize-recover=all -O2 -I. -I./src -I../src -I.. -o ./build/LFUCache ./LFUCache.cc && ./build/LFUCache
*/
#include <iostream>
...
class LFUCache {
	...
	size_t _capacity;
	int _min_freq;
	unordered_map<int, list<int>> _freq_map;  // freq->key
	unordered_map<int, Item> _map;  // key->item

	void visit(unordered_map<int, Item>::iterator it) {
		auto& item = it->second;
		auto fit = _freq_map.find(item.freq);
		assert(fit != _freq_map.end());
		fit->second.erase(item.it);
		if (fit->second.empty()) {
			_freq_map.erase(fit);
		}
		if (_freq_map.find(_min_freq) == _freq_map.end()) {
			_min_freq = _min_freq + 1;
		}
		++item.freq;
		_freq_map[item.freq].push_front(it->first);
		item.it = _freq_map[item.freq].begin();
	}
	...
	void put(int key, int value) {
		...
		if (it != _map.end()) {
			...
			if (_map.size() >= _capacity) {
				int remove_key = _freq_map[_min_freq].back();
				it = _map.find(remove_key);
				assert(it != _map.end());
				auto& item = it->second;
				auto fit = _freq_map.find(item.freq);
				assert(fit != _freq_map.end());
			...
			_freq_map[1].push_front(key);
			_map[key] = {value, 1, _freq_map[1].begin()};
			_min_freq = 1;
		}
	}
};
